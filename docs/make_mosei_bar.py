...
def fig_multi_metric():
    ...
    ax.set_ylabel("Acc-7 (%)")
    ax.set_ylim(min(a7) - 1, max(a7) + 1.5)
    ...
    ax.set_ylabel("Score (%)")
    y_lo = min(min(a2), min(f)) - 1
    y_hi = max(max(a2), max(f)) + 1
    ...
    ax.set_ylim(min(me) - 0.02, max(me) + 0.04)
    ...
    ax.set_ylim(min(co) - 0.02, max(co) + 0.02)
