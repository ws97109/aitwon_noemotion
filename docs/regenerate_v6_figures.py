"""SACF v6 figures — conceptual diagrams only, no formulas.

User requirements:
  • Diagrams must NOT contain mathematical formulas — describe what each
    component does using natural English text only.
  • No text overflow out of boxes; no overlap between text and figure elements.
  • All labels in English (no underscore-as-subscript notation in displayed text).

Detailed equations stay in the docx body text (using OMML).  These figures
only serve as conceptual flowcharts.

Run:  python3 docs/regenerate_v6_figures.py
"""
import warnings
...
# ════════════════════════════════════════════════════════════════════════════
def fig_arch():
    fig, ax = plt.subplots(figsize=(14.0, 13.6))
    ax.set_xlim(0, 14); ax.set_ylim(0, 15.6); ax.axis("off")

    ax.text(7, 15.10, "SACFFinalModel — Multi-Branch Single Model Architecture",
    ...
    # Row 1: Inputs
    bg(ax, 0.4, 13.10, 13.2, 1.2, "#FEF3C7", C["accent"])
    ax.text(7, 14.05, "(i)   Multimodal Inputs",
            ha="center", fontsize=11, color=C["accent"], fontweight="bold")
    rbox(ax, 2.5, 13.50, 3.6, 0.55, C["primary"],
         "Text  (transcript + prompt)", fs=9.5, bold=True)
    rbox(ax, 7.0, 13.50, 3.6, 0.55, C["accent"],
         "Audio  (5-dim acoustic features)", fs=9.5, bold=True)
    rbox(ax, 11.5, 13.50, 3.6, 0.55, C["success"],
         "Vision  (20-dim facial features)", fs=9.5, bold=True)

    # Row 2: Shared encoders
    bg(ax, 0.4, 11.10, 13.2, 1.55, "#DBEAFE", C["primary"])
    ax.text(7, 12.45, "(ii)   Shared Encoders",
            ha="center", fontsize=11, color=C["primary"], fontweight="bold")
    rbox(ax, 2.5, 11.70, 3.6, 0.8, C["primary"],
         "DeBERTa-v3-large\n(24-layer Transformer)", fs=9, bold=True)
    rbox(ax, 7.0, 11.70, 3.6, 0.8, C["accent"],
         "Bidirectional LSTM\nfor audio sequence", fs=9, bold=True)
    rbox(ax, 11.5, 11.70, 3.6, 0.8, C["success"],
         "Bidirectional LSTM\nfor vision sequence", fs=9, bold=True)
    for cx in (2.5, 7.0, 11.5):
        arr(ax, cx, 13.20, cx, 12.13)
    ...
    ax.text(7, branch_top + 0.05,
            "(iii)   Four Parallel Branches    (independent parameters; dropout = 0.10 / 0.20 / 0.30 / 0.40)",
            ha="center", fontsize=10, color=C["purple"], fontweight="bold")

    for cx in (2.0, 5.0, 8.0, 11.0):
        arr(ax, 2.5, 11.30, cx, 10.30, color=C["primary"], lw=0.5, hw=0.10)
        arr(ax, 7.0, 11.30, cx, 10.30, color=C["accent"], lw=0.5, hw=0.10)
        arr(ax, 11.5, 11.30, cx, 10.30, color=C["success"], lw=0.5, hw=0.10)
    ...
    for i in range(4):
        ...
        rbox(ax, cx, 8.60, 2.4, 0.5, branch_colors[i],
             "Sentiment-Aware\nCross-modal Fusion", fs=8, bold=True)
    ...
    rbox(ax, 3.0, 4.15, 3.6, 0.7, C["primary"],
         "TTA × 5\n(stochastic test-time averaging)", fs=8.5, bold=True)
    rbox(ax, 7.0, 4.15, 3.6, 0.7, C["accent"],
         "3-Seed Ensemble\n(arithmetic mean of 3 runs)", fs=8.5, bold=True)
    rbox(ax, 11.0, 4.15, 3.6, 0.7, C["success"],
         "Reg-Cls Probability Fusion\n(classification + regression)",
         fs=8.5, bold=True)
...
def fig_pea():
    ...
    ax.text(5.4, 3.45, "(produces a 0–1 score per token)",
            ha="center", fontsize=8, color=C["muted"], style="italic")
...
def fig_sacf_steps():
    ...
    ax.text(7, 6.60, "Sentiment-Aware Cross-modal Fusion (SACF) — Four Steps",
            ha="center", fontsize=13.5, fontweight="bold", color=C["primary"])
    ax.text(7, 6.15,
    ...
    for i, (title, body, out, col) in enumerate(
        ...
        # Body description (plain text, no formulas)
...
def fig_branches():
    ...
    ax.text(7, 7.40, "Four Parallel Branches — Sources of Diversity",
            ha="center", fontsize=13.5, fontweight="bold", color=C["primary"])
    ax.text(7, 6.95,
            "Three mechanisms keep the branches diverse so that their "
            "average behaves like a multi-model ensemble",
            ha="center", fontsize=9.5, color=C["muted"], style="italic")
    ...
    for i in range(4):
        ...
        rbox(ax, cx, 3.05, bw - 0.4, 0.42, branch_colors[i],
             "Attention → Fusion → Proj", fs=8.5, bold=True)
    ...
    bg(ax, 2.0, 0.10, 10.0, 0.95, "#FFE4E6", C["rose"])
    rbox(ax, 7.0, 0.55, 9.5, 0.55, C["rose"],
         "Internal Mean — arithmetic average of 4 branches  →  Acc-7 ≈ 53.21%",
         fs=9.5, bold=True)
    for i in range(4):
        cx = 1.05 + i * 3.0 + bw/2
        arr(ax, cx, 1.35, 7.0, 1.10, color=branch_colors[i], lw=0.8, hw=0.10)

    save(fig, "v6_fig_branches")


# ════════════════════════════════════════════════════════════════════════════
# FIG loss composition — conceptual, no formulas
# ════════════════════════════════════════════════════════════════════════════
def fig_loss_comp():
    ...
    ax.text(7, 7.65,
            ...
            "aggregation & contrast  →  consistency regularization & total",
            ha="center", fontsize=9.5, color=C["muted"], style="italic")
    ...
    ax.text(5.7, 5.75, "(penalizes distant mis-class.)",
            ha="center", fontsize=8.5, color=C["muted"], style="italic")
    ax.text(9.3, 5.75, "(positive / negative)",
            ha="center", fontsize=8.5, color=C["muted"], style="italic")
    ...
    ax.text(7, 4.45,
            "Layer 2 — Cross-Branch Aggregation, Diversity & Cross-modal Contrast",
    ...
    rbox(ax, 7.0, 2.85, 12.0, 0.50, C["danger"],
         "Cross-modal contrast — pull text/audio/vision embeddings "
         "together for the same sample, push apart for different samples",
         fs=9.5, bold=True)

    # Layer 3
    bg(ax, 0.4, 0.3, 13.2, 1.85, "#DCFCE7", C["success"])
    ax.text(7, 1.95,
            "Layer 3 — Consistency Regularization & Final Total",
            ha="center", fontsize=11, fontweight="bold", color=C["success"])
    rbox(ax, 7.0, 1.30, 12.4, 0.55, C["success"],
         "R-Drop consistency — two random forward passes "
...
def fig_train_timeline():
    ...
    ax.text(78, 1.1, "run weights",
            ha="center", fontsize=8.5, color=C["primary"], fontweight="bold")

    bg(ax, 0.5, -1.7, 78, 2.6, "#EDE9FE", C["purple"], alpha=0.30)
    ax.text(40, 0.60, "Multi-Seed Ensemble  (three independent runs)",
            ha="center", fontsize=10.5, fontweight="bold", color=C["purple"])
    ...
    rbox(ax, 40, -1.65, 36, 0.50, C["danger"],
         "Final weights  =  parameter-space average of the three runs",
         fs=10, bold=True)

    save(fig, "v6_fig_train_timeline")
...
def fig_inference():
    ...
    ax.text(7, 6.40,
            "Three layers of variance reduction without using any test-set "
            "statistics",
            ha="center", fontsize=9.5, color=C["muted"], style="italic")

    # Stage 1
    bg(ax, 0.3, 4.2, 4.2, 1.95, "#DBEAFE", C["primary"])
    ax.text(2.4, 5.85, "Stage 1   TTA × 5",
            ha="center", fontsize=11, fontweight="bold", color=C["primary"])
    for i in range(5):
    ...
    ax.text(7.0, 5.85, "Stage 2   3-Seed Ensemble",
            ha="center", fontsize=11, fontweight="bold", color=C["accent"])
    seeds = [("seed 42", C["primary"]),
             ("seed 123", C["accent"]),
             ("seed 2024", C["success"])]
    for i, (lbl, col) in enumerate(seeds):
        rbox(ax, 5.5 + i * 1.10, 4.95, 0.95, 0.45, col, lbl, fs=8, bold=True)
    ax.text(7.0, 4.40,
    ...
    ax.text(11.7, 5.85, "Stage 3   Reg-Cls Fusion",
            ha="center", fontsize=11, fontweight="bold", color=C["success"])
    ax.text(11.7, 5.10,
    ...
    flow = [
        ...
        ("Three\nforward passes\n(per seed)", C["accent"], 3.4, 2.0),
        ("Per-run TTA\n(five passes\nwith dropout)", C["primary"], 5.7, 2.0),
        ("Cross-run\narithmetic mean\nof logits", C["secondary"], 8.0, 2.0),
...
def fig_distribution():
    ...
    plt.tight_layout()
    save(fig, "v6_fig_distribution")


# ════════════════════════════════════════════════════════════════════════════
# FIG regcls — example bar charts (kept; these show actual probability data,
# not abstract formulas; titles simplified to descriptions)
# ════════════════════════════════════════════════════════════════════════════
def fig_regcls():
    ...
    fig.suptitle("Reg-Cls Probability Fusion — Example Test Sample",
                 fontsize=12, fontweight="bold", y=1.02)
    fig.tight_layout()
    save(fig, "v6_fig_regcls")
...
def fig_radar():
    ...
    fig.suptitle("SACFFinalModel — Overall Performance",
                 fontsize=12.5, fontweight="bold", y=1.02)
    fig.tight_layout()
    save(fig, "v6_fig_radar")


if __name__ == "__main__":
    print("Regenerating v6 conceptual figures (no formulas, no overlap) ...")
    fig_arch();             print("  ✓ v6_fig_arch")
    fig_pea();              print("  ✓ v6_fig_pea")
    fig_sacf_steps();       print("  ✓ v6_fig_sacf_steps")
    fig_branches();         print("  ✓ v6_fig_branches")
    fig_loss_comp();        print("  ✓ v6_fig_loss_comp")
    fig_train_timeline();   print("  ✓ v6_fig_train_timeline")
    fig_inference();        print("  ✓ v6_fig_inference")
    fig_distribution();     print("  ✓ v6_fig_distribution")
    fig_regcls();           print("  ✓ v6_fig_regcls")
    fig_radar();            print("  ✓ v6_fig_radar")
    print("Done.")
