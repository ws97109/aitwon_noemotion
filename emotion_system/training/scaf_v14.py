...
def main():
    print("=" * 70)
    print("MOSI 多模態情感分析 v10 — Hybrid")
    print("scaf_old 穩定架構 + Transformer編碼器 + Focal Loss + 漸進解凍")
    print("=" * 70)
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if metrics["Acc7"] > best_acc7["Acc7"]:
            ...
            torch.save({
                ...
                "metrics": metrics,
                "config": config,
            }, save_dir / "v14_best_acc7.pth")
            print(f"  ✅ 新最佳 Acc7={metrics['Acc7']:.2f}%")

    # 測試集
    print("\n" + "=" * 60)
    ckpt = torch.load(save_dir / "v14_best_acc7.pth", map_location=device, weights_only=False)
    ...
    with open(save_dir / "v14_history.json", "w") as f:
        json.dump({
            "history": history,
