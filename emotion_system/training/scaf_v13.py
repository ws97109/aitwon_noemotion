"""
MOSI 多模態情感分析 v13 — 精簡高效版
基於 v10 核心 (49.27%) + 精準的激進優化
...
v13 改進策略：
1. 保留 v10 穩定架構✓
2. 提高 cls7 權重：3.0 → 4.0✓
3. 更大 batch size：20 → 32✓
4. 輕量 Mixup（20%機率）✓
5. 更激進學習率✓
6. 延長訓練：50 → 65 epochs✓
...
def train_epoch(model, loader, criterion, optimizer, scheduler, device, scaler):
    ...
    for batch in tqdm(loader, desc="Train", leave=False):
        ...
        # v13 新增：輕量 Mixup（20%機率，僅非語言模態）
        if random.random() < 0.2:
            batch_size = aud.size(0)
            lam = np.random.beta(0.2, 0.2)
            index = torch.randperm(batch_size).to(device)
            aud = lam * aud + (1 - lam) * aud[index]
            vis = lam * vis + (1 - lam) * vis[index]
...
def main():
    print("=" * 70)
    print("MOSI 多模態情感分析 v13 — 精簡高效版")
    print("v10核心 + 激進優化（Batch32 + cls7=4.0 + Mixup20%）")
    print("=" * 70)
    ...
    config = {
        ...
        "num_classes": 7,
        "dropout": 0.25,
        "batch_size": 32,  # 20→32：更穩定梯度
        "num_epochs": 65,  # 50→65：延長訓練
        "lang_lr": 2.5e-6,  # 2e-6→2.5e-6：稍微激進
        "other_lr": 1e-4,  # 8e-5→1e-4：稍微激進
        "weight_decay": 2e-2,
        "warmup_ratio": 0.15,
        "alpha": 4.0,  # 3.0→4.0：更高cls7權重
        "beta": 0.5,   # cls2 權重
        "gamma": 0.3,  # reg 權重
        "mixup_prob": 0.2,  # 新增：輕量Mixup
        "mixup_alpha": 0.2,  # 新增：Mixup強度
    }
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if metrics["Acc7"] > best_acc7["Acc7"]:
            ...
            torch.save({
                ...
                "metrics": metrics,
                "config": config,
            }, save_dir / "v13_best_acc7.pth")
            print(f"  ✅ 新最佳 Acc7={metrics['Acc7']:.2f}%")

    # 測試集
    print("\n" + "=" * 60)
    ckpt = torch.load(save_dir / "v13_best_acc7.pth", map_location=device)
    ...
    with open(save_dir / "hybrid_v10_history.json", "w") as f:
        json.dump({
            "history": history,
