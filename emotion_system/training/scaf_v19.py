"""
MOSI 多模態情感分析 v19 — 架構優化版
基於 v15 進行模組改進，提升測試集泛化能力

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
v15 分析：
✅ Val Acc7: 55.90% (驗證集表現優異)
❌ Test Acc7: 47.81% (測試集表現差)
❌ Gap: 8.09% (嚴重過擬合)

v19 架構優化：
1. ✅ 簡化 CrossModalAttention：移除 Gate，減少過擬合點
2. ✅ 縮小 Fusion 維度：512 → 384 (減少 25% 參數)
3. ✅ Multi-Sample Dropout：推理時多次採樣增強穩健性
4. ✅ 添加 BatchNorm：關鍵層加入 BN 提升泛化
5. ✅ Temperature Scaling：輸出層溫度校準
6. ✅ 保留 v15 最佳配置：lr=5e-6, freeze=6, bs=8, epochs=70

目標：Val Acc7 > 54%, Test Acc7 > 50.44%, Gap < 4%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
def main():
    print("=" * 70)
    print("MOSI 多模態情感分析 v19 — 架構優化版")
    print("簡化模組 + BatchNorm + Temperature Scaling")
    print("=" * 70)
    ...
    config = {
        ...
        "vision_dim": 20,
        "modal_hidden": 256,
        "fusion_dim": 384,  # v19: 512 → 384 (減少 25% 參數)
        "num_classes": 7,
        "dropout": 0.2,  # 保持 v15 配置
        "batch_size": 8,  # 保持 v15 配置
        "num_epochs": 70,  # 保持 v15 配置
        "lang_lr": 5e-6,  # 保持 v15 配置
        "other_lr": 1e-4,  # 保持 v15 配置
        "weight_decay": 1e-2,  # 保持 v15 配置
        "warmup_ratio": 0.1,
        "alpha": 3.0,  # cls7 權重
        "beta": 0.5,   # cls2 權重
        "gamma": 0.3,  # reg 權重
        "temperature": 1.5,  # v19: 輸出層溫度校準
    }
    ...
    print(f"\n開始訓練 | 設備: {device}")
    print(f"[策略] Transformer編碼器 + Focal Loss + 漸進解凍 + Acc7最大化\n")
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if metrics["Acc7"] > best_acc7["Acc7"]:
            ...
            torch.save({
                ...
                "metrics": metrics,
                "config": config,
            }, save_dir / "v19_best_acc7.pth")
            print(f"  ✅ 新最佳 Acc7={metrics['Acc7']:.2f}%")

    # 測試集
    print("\n" + "=" * 60)
    ckpt = torch.load(save_dir / "v19_best_acc7.pth", map_location=device, weights_only=False)
    ...
    with open(save_dir / "v15_history.json", "w") as f:
        json.dump({
            "history": history,
