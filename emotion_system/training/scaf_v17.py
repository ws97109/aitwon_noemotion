"""
MOSI 多模態情感分析 v17 — 平衡優化版
在性能和泛化之間找到最佳平衡點

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
版本對比：
v15: Val 55.90%, Test 47.81%, gap 8.09% → 過擬合嚴重
v16: Val 52.84%, Test 48.25%, gap 4.59% → 正則化太強

v17 平衡策略（取中間值）：
1. ✅ Early Stopping (patience=7↑, min_delta=0.3%)
2. ✅ 適度正則化 (dropout=0.25, weight_decay=0.015)
3. ✅ 平衡學習率 (lang_lr=4.5e-6, other_lr=9e-5)
4. ✅ 中等 Label Smoothing (focal=0.12, cls2=0.075)
5. ✅ 延長訓練時間 (70 epochs)
6. ✅ 梯度裁剪 (max_norm=1.0)

目標：Test Acc7 > 50.44% (當前 48.25%)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
class HybridLoss(nn.Module):
    def __init__(self, class_weights, alpha=3.0, beta=0.5, gamma=0.3):
        ...
        self.beta = beta
        self.gamma = gamma
        # v17: 中等 label smoothing (v15: 0.1, v16: 0.15)
        self.focal = FocalLoss(alpha=class_weights, gamma=2.0, label_smoothing=0.12)
        self.cls2 = nn.CrossEntropyLoss(label_smoothing=0.075)  # (v15: 0.05, v16: 0.1)
...
def main():
    print("=" * 70)
    print("MOSI 多模態情感分析 v17 — 平衡優化版")
    print("在性能和泛化之間找到最佳平衡點")
    print("=" * 70)
    ...
    config = {
        ...
        "fusion_dim": 512,
        "num_classes": 7,
        "dropout": 0.25,  # v17: 平衡值 (v15: 0.2, v16: 0.3)
        "batch_size": 8,
        "num_epochs": 70,  # v17: 恢復較長訓練 (v15: 70, v16: 50)
        "lang_lr": 4.5e-6,  # v17: 中間值 (v15: 5e-6, v16: 4e-6)
        "other_lr": 9e-5,  # v17: 中間值 (v15: 1e-4, v16: 8e-5)
        "weight_decay": 1.5e-2,  # v17: 中間值 (v15: 1e-2, v16: 2e-2)
        "warmup_ratio": 0.1,
        "alpha": 3.0,  # cls7 權重
        "beta": 0.5,   # cls2 權重
        "gamma": 0.3,  # reg 權重
        # v17 Early Stopping: 更寬容的設置
        "patience": 7,  # 7 epochs 無改善則停止（v16: 5）
        "min_delta": 0.003,  # 最小改善幅度 0.3%（v16: 0.5%）
    ...
    print(f"\n開始訓練 | 設備: {device}")
    print(f"[v17策略] Early Stop (p={config['patience']}) + 平衡正則化 + 延長訓練\n")
