...
import pickle, random, os, copy, argparse, json
import torch
...
def train_one_seed(seed, train_loader, val_loader, test_loader,
    ...
    # ★ MMAFFIn 預訓練 backbone 載入（若提供）
    pre_path = config.get("pretrained_backbone")
    if pre_path:
        ckpt = torch.load(pre_path, map_location="cpu")
        state = ckpt.get("backbone_state_dict", ckpt)
        missing, unexpected = model.lang_backbone.load_state_dict(state, strict=False)
        print(f"  [Pretrain] Loaded backbone from {pre_path}")
        print(f"  [Pretrain] missing={len(missing)} unexpected={len(unexpected)} "
              f"pretrain_val_acc={ckpt.get('best_val_acc', '?')}")
        if len(missing) > 0: print(f"  [Pretrain]   missing[0:3]={missing[:3]}")
        if len(unexpected) > 0: print(f"  [Pretrain]   unexpected[0:3]={unexpected[:3]}")
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if trainval_mode:
            ...
            epoch_log.append({"seed": seed, "epoch": ep1, "train_loss": round(float(loss), 6)})
        else:
            ema.apply_shadow()
            ...
            epoch_log.append({"seed": seed, "epoch": ep1, "train_loss": round(float(loss), 6),
                              "val_acc7": val_m["Acc7"], "val_acc2": val_m["Acc2"],
                              "val_f1": val_m["F1"], "val_mae": val_m["MAE"], "val_corr": val_m["Corr"]})
    ...
    if trainval_mode:
        ...
        return 0.0, None, swa_test_out, swa_state, epoch_log
    else:
        ...
        return best_val_acc7, chosen_val_out, chosen_test_out, chosen_state, epoch_log
...
def main():
    ...
    cli.add_argument("--pretrained_backbone", default=None,
                     help="Path to .pt file produced by pretrain_backbone.py")
    cli.add_argument("--version", default=None,
                     help="Override version tag for output files (default: v60)")
    cli.add_argument("--gpu", default=None,
    ...
    if cli_args.pretrained_backbone:
        print(f"  [MMAFFIn] pretrained backbone = {cli_args.pretrained_backbone}")
    if cli_args.version:
        print(f"  [MMAFFIn] version tag         = {cli_args.version}")
    print("=" * 70)
    ...
    config["pretrained_backbone"] = cli_args.pretrained_backbone
    ...
    # ★ Per-seed 斷點續跑：每個 seed 完成立即落盤，下次啟動可跳過已完成 seed
    per_seed_dir = MODEL_DIR / f"_partial_{config['version']}"
    per_seed_dir.mkdir(exist_ok=True, parents=True)

    completed_seeds = {}
    for s in config["seeds"]:
        p = per_seed_dir / f"seed{s}.npz"
        if p.exists():
            try:
                ...
                completed_seeds[s] = {
                    ...
                    "test_l2":  z["test_l2"],
                    "test_reg": z["test_reg"],
                    "val_acc":  float(z["val_acc"]),
                }
                print(f"[RESUME] ✓ Seed {s} already complete (loaded from {p})")
            except Exception as e:
                print(f"[RESUME] ⚠ failed to load {p}: {e}; will retrain")

    for seed in config["seeds"]:
        if seed in completed_seeds:
            c = completed_seeds[seed]
            all_test_l7.append(c["test_l7"])
            all_test_l2.append(c["test_l2"])
            all_test_reg.append(c["test_reg"])
            seed_results.append({"seed": seed, "trainval_mode": use_trainval,
                                 "best_val_acc7": round(c["val_acc"], 2),
                                 "resumed_from_disk": True})
            # model state for this seed lives at per_seed_dir/sacf_seed{N}.pt
            seed_model_states.append({"seed": seed, "val_acc7": c["val_acc"],
                                      "state_dict_path": str(per_seed_dir / f"sacf_seed{seed}.pt")})
            continue

        val_acc, val_out, test_out, model_state, epoch_log = train_one_seed(
        ...
        # ★ 立即落盤 per-seed 成果（OOM-kill 防護）
        try:
            np.savez(str(per_seed_dir / f"seed{seed}.npz"),
                     test_l7=test_out[0], test_l2=test_out[1], test_reg=test_out[2],
                     val_acc=np.array(val_acc, dtype=np.float32))
            torch.save({"seed": seed, "version": config["version"],
                        "val_acc7": round(val_acc, 4),
                        "state_dict": model_state},
                       str(per_seed_dir / f"sacf_seed{seed}.pt"))
            # per-seed epoch log
            with open(per_seed_dir / f"epoch_log_seed{seed}.json", "w", encoding="utf-8") as f:
                json.dump({"seed": seed, "epochs": epoch_log}, f, ensure_ascii=False)
            print(f"[CHECKPOINT] ✓ Seed {seed} saved to {per_seed_dir}")
        except Exception as e:
            print(f"[CHECKPOINT] ⚠ failed to save seed {seed}: {e}")
    ...
    # ─────────────────────────────────────────────────────
    # ★ 每 epoch 訓練 log（供 A/B 曲線視覺化）
    # ─────────────────────────────────────────────────────
    epoch_log_file = MODEL_DIR / f"epoch_log_{config['version']}.json"
    with open(epoch_log_file, "w", encoding="utf-8") as f:
        json.dump({
            "version": config["version"],
            "pretrained_backbone": config.get("pretrained_backbone"),
            "seeds":   config["seeds"],
            "epochs":  all_epoch_logs,
        }, f, indent=2, ensure_ascii=False)
    print(f"Epoch log: {epoch_log_file}")
    ...
    for entry in seed_model_states:
        ...
        # 支援兩種：內存中的 state_dict 或從 per_seed_dir 載入
        if "state_dict" in entry:
            state = entry["state_dict"]
        else:
            state = torch.load(entry["state_dict_path"], map_location="cpu")["state_dict"]
    ...
    best_state = (best_seed_entry["state_dict"] if "state_dict" in best_seed_entry
                  else torch.load(best_seed_entry["state_dict_path"], map_location="cpu")["state_dict"])
    torch.save({
        "model_config":     model_config_meta,
