"""
MOSI 多模態情感分析 v18 — 激進防過擬合版
解決持續的 Val-Test gap 問題

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
版本演進：
v15: Val 55.90%, Test 47.81%, gap 8.09% → 嚴重過擬合
v16: Val 52.84%, Test 48.25%, gap 4.59% → 過度正則化，性能受限
v17: Val 53.71%, Test 48.54%, gap 5.17% → 仍有 5% gap，未達目標

核心問題：模型在驗證集上過擬合，泛化到測試集時性能下降

v18 激進策略：
1. ✅ 更強正則化 (dropout=0.35, weight_decay=0.03)
2. ✅ 數據增強 (Gaussian noise for audio/vision)
3. ✅ 更保守學習率 (lang_lr=3e-6, 更慢更穩定的學習)
4. ✅ 更寬容 Early Stop (patience=10, 給更多探索機會)
5. ✅ 增強 Label Smoothing (focal=0.15, 防止過度自信)
6. ✅ Dropout on LayerNorm
7. ✅ 訓練集 Mixup 增強
...
def main():
    print("=" * 70)
    print("MOSI 多模態情感分析 v18 — 激進防過擬合版")
    print("更強正則化 + 數據增強 + 保守學習")
    print("=" * 70)
    ...
    config = {
        ...
        "fusion_dim": 512,
        "num_classes": 7,
        "dropout": 0.35,  # v18: 激進正則化 (v17: 0.25)
        "batch_size": 8,
        "num_epochs": 80,  # v18: 更長訓練探索更多可能
        "lang_lr": 3e-6,  # v18: 更保守學習率 (v17: 4.5e-6)
        "other_lr": 7e-5,  # v18: 更保守 (v17: 9e-5)
        "weight_decay": 3e-2,  # v18: 更強權重衰減 (v17: 1.5e-2)
        "warmup_ratio": 0.15,  # v18: 更長 warmup
        "alpha": 3.0,  # cls7 權重
        "beta": 0.5,   # cls2 權重
        "gamma": 0.3,  # reg 權重
        # v18 Early Stopping: 更寬容，給更多機會
        "patience": 10,  # 10 epochs 無改善則停止（v17: 7）
        "min_delta": 0.002,  # 最小改善幅度 0.2%（更敏感）
        "max_grad_norm": 1.0,  # 梯度裁剪
        # v18 新增：數據增強
        "noise_std": 0.05,  # Gaussian noise 標準差
        "mixup_alpha": 0.2,  # Mixup 參數
    }
    ...
    print(f"\n開始訓練 | 設備: {device}")
    print(f"[v18策略] 激進防過擬合 | dropout={config['dropout']} | wd={config['weight_decay']} | p={config['patience']}\n")
    ...
    # 測試集
    print("\n" + "=" * 60)
    ckpt = torch.load(save_dir / "v18_best_acc7.pth", map_location=device, weights_only=False)
