"""
MOSI 多模態情感分析 v21 — 全面優化版
架構改進 + 訓練策略優化 + 高級正則化

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
版本演進分析：
v15: Val 55.90%, Test 48.25%, Gap 7.65% → 高性能但過擬合
v16: Val 52.84%, Test 48.25%, Gap 4.59% → 改善 gap 但性能受限
v17: Val 53.71%, Test 48.54%, Gap 5.17% → 中等效果
v19: 架構優化（fusion 384, BatchNorm, Temperature）

v21 綜合優化策略：

【架構優化】（基於 v19）
1. ✅ 簡化 CrossModalAttention：移除 Gate 機制
2. ✅ 縮小 Fusion 維度：512 → 320 (更激進，減少 37.5%)
3. ✅ BatchNorm + LayerNorm 混合：提升泛化穩定性
4. ✅ Temperature Scaling：輸出校準 (T=1.8)
5. ✅ Residual Dropout：隨機跳過殘差連接

【訓練策略優化】
1. ✅ Stochastic Weight Averaging (SWA)：平滑模型參數
2. ✅ Cosine Annealing：學習率週期性調整
3. ✅ Gradient Accumulation：模擬 bs=32 (4 steps)
4. ✅ Gaussian Noise 數據增強：audio/vision 加噪
5. ✅ Early Stopping：patience=8, 防止過度訓練
6. ✅ Warmup + Cosine Decay：更穩定的學習

【正則化增強】
1. ✅ Dropout: 0.3 (較強)
2. ✅ Weight Decay: 0.02 (較強)
3. ✅ Label Smoothing: 0.15 (focal), 0.1 (cls2)
4. ✅ Gradient Clipping: max_norm=1.0

目標：Test Acc7 > 50.44%, Val-Test Gap < 3%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
class CrossModalAttention(nn.Module):
    ...
    def __init__(self, lang_dim: int, modal_dim: int, dropout: float = 0.1):
        ...
        # v19: 簡化的融合層，添加 BatchNorm
        self.fusion = nn.Sequential(
            nn.Linear(lang_dim, lang_dim),
            nn.BatchNorm1d(lang_dim),  # v19: 添加 BN
            nn.ReLU(),
            nn.Dropout(dropout),
        )
...
class HybridModel(nn.Module):
    def __init__(
        ...
        fusion_dim: int = 384,  # v19: 512 → 384 (減少 25% 參數)
        num_classes: int = 7,
        dropout: float = 0.2,
        temperature: float = 1.5,  # v19: 添加溫度參數
    ):
        super().__init__()
        ...
        # v19: 添加 BatchNorm 提升泛化
        self.shared = nn.Sequential(
            nn.Linear(lang_dim, fusion_dim),
            nn.BatchNorm1d(fusion_dim),  # v19: 添加 BN
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(fusion_dim, fusion_dim),  # v19: 額外變換層
    ...
    def forward(self, input_ids, attention_mask, audio, audio_mask, vision, vision_mask):
        ...
        # v19: Temperature Scaling 校準輸出置信度
        logits7 = self.cls7_head(feat) / self.temperature
        logits2 = self.cls2_head(feat) / self.temperature
...
def train_epoch(model, loader, criterion, optimizer, scheduler, device, scaler,
                accumulation_steps=1, noise_std=0.0):
    """
    v21: 添加 Gradient Accumulation 和數據增強
    """
    model.train()
    total_loss = 0.0
    optimizer.zero_grad()
    ...
    for i, batch in enumerate(tqdm(loader, desc="Train", leave=False)):
        ...
        # v21: 數據增強 - Gaussian noise
        if noise_std > 0 and model.training:
            aud = aud + torch.randn_like(aud) * noise_std
            vis = vis + torch.randn_like(vis) * noise_std

        use_amp = scaler is not None
        with torch.cuda.amp.autocast(enabled=use_amp):
            l7, l2, reg = model(ids, mask, aud, amask, vis, vmask)
            loss, _ = criterion(l7, l2, reg, cl7, cl2, rl)
            # v21: Gradient Accumulation - 損失縮放
        ...
        # v21: 每 accumulation_steps 步更新一次
...
def main():
    print("=" * 70)
    print("MOSI 多模態情感分析 v21 — 全面優化版")
    print("架構優化 + SWA + Cosine Annealing + 數據增強")
    print("=" * 70)
    ...
    config = {
        ...
        "vision_dim": 20,
        "modal_hidden": 256,
        "fusion_dim": 320,  # v21: 512 → 320 (減少 37.5%)
        "num_classes": 7,
        "dropout": 0.3,  # v21: 0.2 → 0.3 (更強正則化)
        "batch_size": 8,  # 實際 bs, 使用 gradient accumulation 模擬 32
        "num_epochs": 80,  # v21: 延長訓練探索 SWA
        "lang_lr": 4e-6,  # v21: 5e-6 → 4e-6 (更保守)
        "other_lr": 8e-5,  # v21: 1e-4 → 8e-5 (更保守)
        "weight_decay": 2e-2,  # v21: 1e-2 → 2e-2 (更強)
        "warmup_ratio": 0.15,  # v21: 0.1 → 0.15 (更長 warmup)
        "alpha": 3.0,  # cls7 權重
        "beta": 0.5,   # cls2 權重
        "gamma": 0.3,  # reg 權重
        "temperature": 1.8,  # v21: 1.5 → 1.8 (更強校準)
        # v21 新增：訓練策略
        "gradient_accumulation_steps": 4,  # 模擬 bs=32
        "swa_start": 50,  # SWA 從第 50 epoch 開始
        "swa_lr": 1e-6,  # SWA 學習率
        "noise_std": 0.05,  # Gaussian noise 標準差
        "patience": 8,  # Early stopping patience
        "min_delta": 0.002,  # 最小改善 0.2%
    ...
    model = HybridModel(
        ...
        temperature=config["temperature"],  # v19: 添加溫度參數
    ).to(device)
    ...
    print(f"\n開始訓練 | 設備: {device}")
    print(f"[v21策略] SWA + Cosine Annealing + Grad Accum + 數據增強\n")
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if epoch >= config["swa_start"]:
            ...
            swa_n += 1
            print(f"  📊 SWA 更新 (n={swa_n})")

        # v21: Early Stopping 檢查
    ...
    # v21: 如果使用了 SWA，保存 SWA 模型
    if swa_model is not None:
        print(f"\n📊 更新 SWA BatchNorm 統計...")
        optim.swa_utils.update_bn(train_loader, swa_model, device)
        torch.save({
            "model_state": swa_model.module.state_dict(),
            "swa_n": swa_n,
            "config": config,
        }, save_dir / "v21_swa_model.pth")
        print(f"✅ SWA 模型已保存 (平均 {swa_n} 個 checkpoints)")

    # 測試集 - 測試最佳模型
    print("\n" + "=" * 60)
    print("【測試最佳模型】")
    ckpt = torch.load(save_dir / "v21_best_acc7.pth", map_location=device, weights_only=False)
    ...
    test_results = {"best_model": test_m}

    # v21: 測試 SWA 模型
    if swa_model is not None:
        print("\n【測試 SWA 模型】")
        _, test_m_swa = validate(swa_model, test_loader, criterion, device)
        print(f"  Acc7 : {test_m_swa['Acc7']:.2f}%")
        print(f"  Acc2 : {test_m_swa['Acc2']:.2f}%")
        print(f"  F1   : {test_m_swa['F1']:.2f}%")
        print(f"  MAE  : {test_m_swa['MAE']:.4f}")
        print(f"  Corr : {test_m_swa['Corr']:.4f}")
        test_results["swa_model"] = test_m_swa

    with open(save_dir / "v21_history.json", "w") as f:
        json.dump({
            "history": history,
