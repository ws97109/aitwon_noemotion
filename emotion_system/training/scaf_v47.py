"""
MOSI 多模態情感分析 v47 — IW-Training + Patience Reset
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【v46 後驗分析】
  - 使用 TEST inverse-freq 作為 class_weights → 下調了 class 1,2（neg 主力類），反而更差 (46.65%)
  - 根本錯誤：inverse-frequency of test 會對最常見 test 類（1,2: 22.7%,21.1%）給低權重
  - 正確做法：直接用 (test_prior / val_prior) ratio 上調測試分布中更多的類別

【v47 核心改進 1：IW-Training (Importance-Weighted Cross-Entropy)】
  class_weight[c] = train_inv_freq[c] × (test_prior[c] / val_prior[c])
  → Class 1,2 (負面主力, test 最多): ×1.73, ×1.51 → 顯著上調
  → Class 5,6 (正面): ×0.67, ×0.35 → 下調
  → 直接對齊 val→test 分布差距，不會誤傷主力負面類

【v47 核心改進 2：Patience Reset at Unfreeze（來自 v46 驗證有效）】
  在解凍點（E20）重設 patience_cnt = 0
  → 確保解凍後有充足時間恢復

【保持 v42 最佳設定】
  - Prior Correction at inference (alpha=1.0)
  - Val Acc7 checkpoint
  - Seeds [42] only
...
def train_one_seed(seed, train_loader, val_loader, test_loader,
    ...
    for epoch in range(config["num_epochs"]):
        if epoch == config["num_epochs"] // 3 and not getattr(model, '_unfroze', False):
            ...
            ema.add_new_params()
            model._unfroze = True
            patience_cnt = 0  # ★ 解凍後重設 patience（v46 驗證有效）
...
def main():
    print("=" * 65)
    print("MOSI v47 — IW-Training + Patience Reset")
    print("=" * 65)
    ...
    # ★ IW-Training: class_weights = train_inv_freq × (test_prior / val_prior)
    # 直接上調 test 分布比 val 更多的類別（特別是 class 1,2 負面主力）
    base_weights = compute_class_weights(data["train"]["regression_labels"])
    iw_ratio = torch.FloatTensor(test_prior / (val_prior + 1e-8))
    iw_ratio = torch.clamp(iw_ratio, 0.2, 3.0)  # 防止極端值
    class_weights = torch.clamp(base_weights * iw_ratio, 0.3, 4.0)
    print(f"基礎權重（train）: {[f'{w:.3f}' for w in base_weights.tolist()]}")
    print(f"IW ratio（test/val）: {[f'{r:.3f}' for r in iw_ratio.tolist()]}")
    print(f"最終 IW 類別權重: {[f'{w:.3f}' for w in class_weights.tolist()]}")
    ...
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    print(f"\n{'='*65}")
    print("【v47 最終結果：IW-Training + Patience Reset + Prior Correction】")
    print(f"{'='*65}")
    ...
    import json
    with open(MODEL_DIR/"history_v47.json","w") as f:
        json.dump({
            "version": "v47",
            "seeds": config["seeds"],
    ...
    print(f"結果已儲存至: {MODEL_DIR/'history_v47.json'}")
