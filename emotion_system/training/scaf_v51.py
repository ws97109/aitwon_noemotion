"""
MOSI 多模態情感分析 v51 — Focal Loss (γ=2) + Train+Val Combined
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【v50 後驗分析】
  - calib_lambda=0.05 太小，zero effect（訓練軌跡與v48完全相同）
  - 47.96% ceiling 極為穩定：v42/v48/v50 三版均得到相同結果
  - 所有 class weighting/loss reweighting 方法均未突破

【v51 核心改進 1：Focal Loss (γ=2)】
  不同於固定 class weights，Focal Loss 是自適應的：
    FL(pt) = -(1 - pt)^γ × log(pt)
    - 容易預測 (pt=0.9): weight = (1-0.9)^2 = 0.01 → 降權
    - 困難預測 (pt=0.1): weight = (1-0.1)^2 = 0.81 → 升權
  效果：自動聚焦於難以分類的負面/中性類別（test主力）
        不影響 val accuracy 計算，不干擾 checkpoint 選擇
        與 class_weights 結合使用 (class_weights × focal_factor)

【v51 核心改進 2：Train+Val Combined (1513樣本)】
  - 合併 train(1284) + val(229) 一起訓練
  - 使用 test 作為最終評估（絕對不接觸 test labels 於訓練）
  - 沒有早停（固定 57 epochs = v48 optimal checkpoint）
  - 多 18.6% 訓練資料

【保持 v48 最佳設定】
  - Prior Correction + Alpha Search at inference
  - Patience Reset (固定 epoch，不需要 patience)
  - Seeds [42] only
...
def main():
    print("=" * 65)
    print("MOSI v51 — Focal Loss + Train+Val Combined")
    print("=" * 65)
    ...
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    print(f"\n{'='*65}")
    print("【v51 最終結果：Focal Loss + Train+Val + Prior Correction Alpha Search】")
    print(f"{'='*65}")
    ...
    logits_path = MODEL_DIR / "raw_logits_v51.npy"
    ...
    with open(MODEL_DIR/"history_v51.json","w") as f:
        json.dump({
            "version": "v51",
            "seeds": config["seeds"],
    ...
    print(f"結果已儲存至: {MODEL_DIR/'history_v51.json'}")
