"""
MOSI 多模態情感分析 v15 — 診斷修復版
基於 scaf_old 的有效配置 + v10 架構

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
關鍵修復（相比v10/v13）：
1. 學習率提升：lang_lr 2e-6 → 5e-6 (來自scaf_old)
2. 解凍層數：12層 → 6層 (來自scaf_old)
3. Batch size：20/32 → 8 (來自scaf_old)
4. 保留v10架構：Transformer編碼器 + Focal Loss
5. 延長訓練：50 → 70 epochs

實驗結果對比：
- v10  Epoch 1: Acc7=4.37%  (lang_lr=2e-6, freeze=12, batch=20)
- v13  Epoch 1: Acc7=6.55%  (lang_lr=2.5e-6, freeze=12, batch=32)
- old  Epoch 1: Acc7=15.72% (lang_lr=5e-6, freeze=6, batch=8)
- v15  期望：>= 20% Acc7 (結合scaf_old配置 + v10改進)

目標：Epoch 10 達到 40%+ Acc7，最終 51%+
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
def main():
    print("=" * 70)
    print("MOSI 多模態情感分析 v15 — 診斷修復版")
    print("scaf_old配置 (lr=5e-6, freeze=6, bs=8) + v10架構")
    print("=" * 70)
    ...
    config = {
        ...
        "fusion_dim": 512,
        "num_classes": 7,
        "dropout": 0.2,  # 0.25 → 0.2 (scaf_old)
        "batch_size": 8,  # 20 → 8 (scaf_old)
        "num_epochs": 70,  # 50 → 70 (延長訓練)
        "lang_lr": 5e-6,  # 2e-6 → 5e-6 (scaf_old, 關鍵修復)
        "other_lr": 1e-4,  # 8e-5 → 1e-4 (scaf_old)
        "weight_decay": 1e-2,  # 2e-2 → 1e-2 (scaf_old)
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if metrics["Acc7"] > best_acc7["Acc7"]:
            ...
            torch.save({
                ...
                "metrics": metrics,
                "config": config,
            }, save_dir / "v15_best_acc7.pth")
            print(f"  ✅ 新最佳 Acc7={metrics['Acc7']:.2f}%")

    # 測試集
    print("\n" + "=" * 60)
    ckpt = torch.load(save_dir / "v15_best_acc7.pth", map_location=device, weights_only=False)
