...
def main():
    ...
    config = {
        ...
        "fusion_dim":   512,
        "num_classes":  7,
        "dropout":      0.3,      # 0.2→0.3 加強正則化
        # 訓練
        "batch_size":   16,       # 8→16 梯度更穩定
        "num_epochs":   30,
        "lang_lr":      5e-6,
        "modal_lr":     1e-4,
        "weight_decay": 5e-2,     # 1e-2→5e-2 加強 L2 正則
        "warmup_ratio": 0.06,
        "freeze_layers":6,
        # 損失
        "alpha":  0.5,
    ...
    if swa_started:
        print("\n[SWA Model 測試集結果]")
        swa_ckpt = torch.load(save_dir / "swa_model.pth", map_location=device)
        model.load_state_dict(swa_ckpt["model_state"])
        _, swa_test_m = validate(model, test_loader, criterion, device)
