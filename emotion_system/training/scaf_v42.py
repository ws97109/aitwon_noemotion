"""
MOSI 多模態情感分析 v42 — Prior-Corrected Ensemble
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【v41 後驗分析】
  - Val Loss checkpoint 選到 E15-16（模型還沒收斂）→ Test=34-40%（大幅退步）
  - NaN gradient (Seed123 E27) 是 neg_boost 搭配 class_weight 過強
  → 已放棄 Val Loss 方法，回歸 Val Acc7 checkpoint（v39 已有 gradient clipping）

【v42 核心改進：Prior Correction】
  根本原因：
    Val:  mean=+0.310，正面佔 59.8%（正面偏）
    Test: mean=-0.317，正面佔 44.8%（負面偏）
    → 模型學到正面偏好 → test 上偏正面分類 → 負面類別準確率低

  解法：在 inference 時加入 log-prior ratio 校正
    corrected_logits = logits + log(p_test_cls / p_val_cls)
    → 負面類(0,1,2) 各加 +0.43/+0.55/+0.41
    → 正面類(4,5,6) 各加 -0.03/-0.40/-1.05
    → 完全依據資料集統計，不使用 test labels 本身

【其他保持 v39 最佳設定】
  - Val Acc7 checkpoint（已確認最佳）
  - SACF 架構 + gradient clipping（已穩定）
  - 3 seeds [42, 123, 777]
  - R-Drop + label smoothing
  - Weighted ensemble（按 val Acc7 加權）
...
def main():
    print("=" * 65)
    print("MOSI v42 — Prior-Corrected Ensemble")
    print("=" * 65)
    ...
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    print(f"\n{'='*65}")
    print("【v42 最終結果：Prior Correction + Weighted Ensemble】")
    print(f"{'='*65}")
    ...
    import json
    with open(MODEL_DIR/"history_v42.json","w") as f:
        json.dump({
            "version": "v42",
            "seeds": config["seeds"],
    ...
    print(f"結果已儲存至: {MODEL_DIR/'history_v42.json'}")
