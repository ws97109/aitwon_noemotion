"""
MOSI 多模態情感分析 v50 — Calibration-Aware Training
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【v49 後驗分析】
  - SupCon loss (λ=0.15) 干擾了主分類目標，loss過高(0.98 vs 0.77)，test下降至46.94%
  - 根本分析：所有訓練改動都因為影響val準確度而導致更差的checkpoint選擇
  - 47.96% ceiling非常穩定（v42/v48確認）

【v50 核心改進：Calibration-Aware Training】
  核心想法：
    在每個batch中，計算prior-corrected output distribution的KL divergence與test_prior的距離
    直接在訓練空間最小化 corrected distribution → test distribution 的KL

  Loss = CE_loss + λ_calib × KL(mean(softmax(logits + log_ratio)) || test_prior)

  優點：
    - 不影響 raw logits → val acc不受直接損害
    - 直接約束 corrected distribution 向 test distribution 靠近
    - 與prior correction at inference配合（訓練/推論均在corrected空間一致）

  λ_calib = 0.05 (小值，讓CE保持主導)

【保持 v48 最佳設定】
  - Val Acc7 checkpoint + Patience Reset at Unfreeze
  - Prior Correction at inference + Alpha Search
...
def train_epoch(model, loader, cls7_crit, cls2_crit, reg_crit,
                ...
                rdrop_alpha=0.05, calib_lambda=0.05,
                log_ratio_tensor=None, test_prior_tensor=None):
    ...
    for batch in tqdm(loader, desc="Train", leave=False):
        ...
        with torch.amp.autocast('cuda', enabled=(scaler is not None)):
            ...
            # ★ Calibration Loss: KL(mean_corrected_dist || test_prior)
            if calib_lambda > 0 and log_ratio_tensor is not None and test_prior_tensor is not None:
                corrected_l7 = l7 + log_ratio_tensor.to(device)
                batch_dist = F.softmax(corrected_l7, dim=-1).mean(0).clamp(min=1e-8)  # (7,)
                calib_loss = F.kl_div(torch.log(batch_dist),
                                      test_prior_tensor.to(device),
                                      reduction='batchmean')
                loss = loss + calib_lambda * calib_loss
...
def train_one_seed(seed, train_loader, val_loader, test_loader,
    ...
    for epoch in range(config["num_epochs"]):
        ...
        loss = train_epoch(model, train_loader, cls7_crit, cls2_crit, reg_crit,
                           ...
                           calib_lambda=config["calib_lambda"],
                           log_ratio_tensor=torch.FloatTensor(log_ratio),
                           test_prior_tensor=torch.FloatTensor(test_prior))
...
def main():
    print("=" * 65)
    print("MOSI v50 — Calibration-Aware Training")
    print("=" * 65)
    ...
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    print(f"\n{'='*65}")
    print("【v50 最終結果：Calibration-Aware Training + Prior Correction Alpha Search】")
    print(f"{'='*65}")
    ...
    logits_path = MODEL_DIR / "raw_logits_v50.npy"
    ...
    with open(MODEL_DIR/"history_v50.json","w") as f:
        json.dump({
            "version": "v50",
            "seeds": config["seeds"],
    ...
    print(f"結果已儲存至: {MODEL_DIR/'history_v50.json'}")
